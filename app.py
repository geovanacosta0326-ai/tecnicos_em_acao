...
with st.sidebar:
    ...
    regiao_sel = st.multiselect("Região FAEC", opcoes_regiao, placeholder="Todas as regiões")

    df_sidebar = df_mapa.copy()
    if regiao_sel:
        df_sidebar = df_sidebar[df_sidebar["regiao_faec"].isin(regiao_sel)]

    #  ALTERADO: Agora lê de df_mapa (Filtro independente)
    opcoes_sindicato = sorted(df_mapa["sindicato"].dropna().unique())
    sindicato_sel = st.multiselect("Sindicato", opcoes_sindicato, placeholder="Todos")

    if sindicato_sel:
        df_sidebar = df_sidebar[df_sidebar["sindicato"].isin(sindicato_sel)]

    #  ALTERADO: Agora lê de df_mapa (Filtro independente)
    opcoes_municipio = sorted(df_mapa["municipio_atual"].dropna().unique())
    municipio_sel = st.multiselect("Município", opcoes_municipio, placeholder="Todos")

    if municipio_sel:
        df_sidebar = df_sidebar[df_sidebar["municipio_atual"].isin(municipio_sel)]

    #  ALTERADO: Agora lê de df_mapa (Filtro independente)
    opcoes_sup = sorted(df_mapa["supervisor"].dropna().unique())
    sup_sel = st.multiselect("Supervisores", opcoes_sup, placeholder="Todos")

    if sup_sel:
        df_sidebar = df_sidebar[df_sidebar["supervisor"].isin(sup_sel)]

    #  ALTERADO: Agora lê de df_mapa (Filtro independente)
    opcoes_tecnico = sorted(df_mapa["tecnico"].dropna().unique())
    tecnico_sel = st.multiselect("Técnico", opcoes_tecnico, placeholder="Todos")
    ...
    projetos_sel = st.multiselect("Projetos", opcoes_projeto, placeholder="Todos")
    ...
    atividades_sel = st.multiselect("Atividades", options_atividade, placeholder="Todas")
...
aba_visao, aba_mapa, aba_equipe, aba_alertas, aba_download, aba_consolidado, aba_historico = st.tabs([
    "📊 Visão Geral", "🗺️ Mapa Operacional", "👥 Equipe & Supervisores", "🚨 Alertas", "📥 Exportar", "📋 Consolidado", "🔄 Histórico"
])
