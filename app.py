...
with st.sidebar:
    ...
    default_max_m = min(23, max_m_all)
    meses_sel = st.slider("⏱️ Tempo de Projeto (meses)", min_m_all, max_m_all, (min_m_all, default_max_m))
    ...
    # Aplica o filtro de Tempo de Projeto logo de início, para que ele
    # já reflita nas opções de todos os filtros em cascata abaixo (e não só no df_f final).
    df_sidebar = df_mapa[
        (df_mapa["tempo_projeto_meses"] >= meses_sel[0]) & (df_mapa["tempo_projeto_meses"] <= meses_sel[1])
    ].copy()
    if regiao_sel:
    ...
    # Atividade (Cadeia Produtiva) — opções dependem dos filtros acima
    ...
    status_sel = st.selectbox("Status do Gap", status_opcoes, index=status_opcoes.index("🟢 Em Dia (≤30 dias)"))
...
with aba_mapa:
    ...
    df_sup_tec = (
        df_f[["supervisor", "tecnico", "municipio_atual", "projeto", "atividade", "tempo_projeto_meses", "data_ultima_visita"]]
    ...
    df_sup_tec["data_ultima_visita"] = pd.to_datetime(df_sup_tec["data_ultima_visita"], errors="coerce").dt.strftime("%d/%m/%Y")
    df_sup_tec.columns = ["Supervisor", "Técnico", "Município", "Projeto", "Cadeia Produtiva", "Tempo de Projeto (meses)", "Última Visita"]
    ...
    linhas_sup_tec = ""
    for _, r in df_sup_tec.iterrows():
        tempo_fmt = "" if pd.isna(r["Tempo de Projeto (meses)"]) else int(r["Tempo de Projeto (meses)"])
        linhas_sup_tec += (
            f'<tr>'
            f'<td>{r["Supervisor"]}</td>'
            f'<td>{r["Técnico"]}</td>'
            f'<td>{r["Município"]}</td>'
            f'<td>{r["Projeto"]}</td>'
            f'<td>{r["Cadeia Produtiva"]}</td>'
            f'<td style="text-align:center;">{tempo_fmt}</td>'
            f'<td>{r["Última Visita"]}</td>'
            f'</tr>'
        )
    ...
    st.markdown(
        ...
        f'<thead><tr>'
        f'<th>Supervisor</th><th>Técnico</th><th>Município</th><th>Projeto</th>'
        f'<th>Cadeia Produtiva</th><th style="text-align:center;">Tempo de Projeto (meses)</th>'
        f'<th>Última Visita</th>'
        f'</tr></thead><tbody>{linhas_sup_tec}</tbody></table></div>',
        unsafe_allow_html=True,
    )
# ════════════════════════════════════════════
